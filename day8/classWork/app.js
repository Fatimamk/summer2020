...
class Task {
    constructor(id, title) {
      ...
      firebase.database().ref("tasks").once('value', (snapshot) => {
          ...
          Object.keys(allTasks).forEach(taskId => {
              ...
              // now tell our html to write each indiv task
          ...
          //takes any function and returns as a string
      })
    }
  }
  ...
  class TaskListPage {
    ...
    addTask(title) {
      ...
      const taskId = firebase.database().ref().child('tasks').push({
          title: title
      }).key;
      //simpler way to do this is on dropbox
